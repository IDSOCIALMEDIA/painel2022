...
echo -e "                             \033[1;31mPainel-SSH V.XX\033[1;36m"
echo ""
...
cd /var/www/html
rm -rf *.php *.png *.ico *.jpg *.sql admin app-assets arquivos assets lib pages phpmailer scripts src > /dev/null 2>&1
...
wget https://github.com/nandoslayer/plusnssh/raw/ntech/painelVXX/autobackup.sh > /dev/null 2>&1 && chmod 777 autobackup.sh && sed -i -e 's/\r$//' autobackup.sh && ./autobackup.sh
...
/etc/init.d/cron reload > /dev/null 2>&1
/etc/init.d/cron restart > /dev/null 2>&1
chmod 777 /var/www/html/admin/pages/apis/backup
...
rm -rf PAINEL01-V.XX.zip PAINEL02-V.XX.zip painelvXX.sh index.html > /dev/null 2>&1
